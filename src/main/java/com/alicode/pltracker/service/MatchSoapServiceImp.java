package com.alicode.pltracker.service;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import com.alicode.pltracker.models.Match;
import com.alicode.pltracker.repository.MatchSoapRepository;

@Component
public class MatchSoapServiceImp implements MatchSoapService {
    private MatchSoapRepository repo;

    @Autowired
    public MatchSoapServiceImp(MatchSoapRepository repo) {
        this.repo = repo;
    }

    @Override
    public List<Match> getMatches(){
        return repo.findAll();
    }

    @Override
    public List<Match> getMatchesByTeam(String team_name, String away_name) {
        return repo.findByhomeTeamOrAwayTeam(team_name, away_name);
    }
}

